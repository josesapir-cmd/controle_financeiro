...
/* ==========================================================================
   Taxa marcada — a taxa de hoje, que a Pluggy nao manda para Tesouro
   ========================================================================== */

export interface CotacaoDeInstrumento {
  fingerprint: string;
  /** O que aparece na coluna: "IPCA + 7,02%". */
  rateLabel: string;
  unitPrice: number | null;
  quotedAt: string;
  note: string | null;
...
export async function listCotacoes(db: Db): Promise<CotacaoDeInstrumento[]> {
  const linhas = await db.query<Record<string, unknown>>(
    "SELECT fingerprint, rate_label, unit_price, quoted_at, note_enc FROM instrument_quotes",
  );

  return linhas.map((linha) => ({
    fingerprint: String(linha.fingerprint),
    rateLabel: String(linha.rate_label),
    unitPrice:
      linha.unit_price === null || linha.unit_price === undefined
        ? null
        : numero(linha.unit_price),
...
/**
 * Marca o instrumento a uma taxa, na data em que ela foi lida.
 *
 * Idempotente pelo nome: marcar de novo atualiza. Duas marcacoes do mesmo papel
 * nao sao historico — sao uma certa e uma velha, e a tela mostraria a errada
 * metade das vezes.
 */
export async function salvarCotacao(
  db: Db,
  nome: string,
  dados: { rateLabel: string; unitPrice?: number | null; quotedAt: string; note?: string | null },
): Promise<void> {
  const rotulo = dados.rateLabel.trim();
  if (!nome.trim() || !rotulo || !dados.quotedAt) return;
...
export async function apagarCotacao(db: Db, nome: string): Promise<void> {
  if (!nome.trim()) return;
  await db.query("DELETE FROM instrument_quotes WHERE fingerprint = $1", [
    instrumentFingerprint(nome),
  ]);
}
